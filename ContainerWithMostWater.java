...
/**
 * Контейнер с наибольшим объёмом воды.
 *
 * <p>Алгоритм реализован на основе однопроходного подхода с учётом положения каждой стенки
 * относительно краёв массива. Введена система штрафов за удалённость от краёв, а также
 * специальное правило для нечётной длины массива, при котором центральный элемент не штрафуется.
 * Это позволяет за один проход выявить два наилучших кандидата — стенки, формирующие контейнер
 * с максимальной площадью (объёмом воды).
 *
 * <p>В отличие от классического метода с двумя указателями, данный подход делает единственный
 * линейный проход и может быть проще для понимания и реализации, сохраняя при этом высокую эффективность.
 *
 * <p>Автор: Соловьёв Владимир Николаевич  
 * Дата рождения: 01.03.1989  
 * Цель: Исследование и проектирование альтернативного линейного алгоритма
 * для задачи определения максимального объёма воды между двумя вертикальными стенками.
 *
 */
